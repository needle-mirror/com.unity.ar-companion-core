...
using ProgressCallback = Unity.AR.Companion.CloudStorage.ProgressCallback;
using UnityObject = UnityEngine.Object;

#if OBJECT_CAPTURE_AVAILABLE && UNITY_EDITOR_OSX
using UnityEditor.XR.ObjectCapture;
#endif
...
namespace Unity.AR.Companion.Core
{
    static class CompanionImportUtils
    {
        ...
        internal static void ImportResource(this IUsesCloudStorage storageUser, CompanionProject project,
            ...
            float lagCompensation, ResourceSyncState state, Action<UnityObject> callback, ProgressCallback progress = null)
        {
            string path;
            string cloudGuid;
            ...
            switch (resource.type)
            {
                ...
                case ResourceType.Environment:
                    requests.Push(storageUser.CloudLoadAsync(resourceKey, true,
                        (success, responseCode, response) =>
                        {
                            if (success)
                            {
                                var environmentName = resource.name;
                                path = EditorUtility.SaveFilePanelInProject(k_SaveEnvironmentDialogTitle,
                                    environmentName,
                                ...
                                var prefab = ImportEnvironment(environmentName, response, path, simulatedPlanePrefab,
                                    simulatedPlaneMaterial, postPrefab, meshPrefab, simulatedLightingPrefab);
                        ...
                        (success, _, response) =>
                        {
                            if (success)
                            {
                                CoroutineUtils.StartCoroutine(storageUser.ImportDataRecording(path,
                                    resourceKey, resource.name, response, lagCompensation, state, callback));
                ...
                case ResourceType.Marker:
                    ...
                    requests.Push(storageUser.CloudLoadAsync(markerKey, true,
                        (success, _, response) =>
                        {
                            if (success)
                ...
                case ResourceType.CapturedObject:
                    requests.Push(storageUser.DownloadCapturedObjectArchive(resourceKey, project.GetLocalPath(),
                        (success, zipFilePath) =>
                        {
                            if (success)
                            {
                                var destinationFolder = $"Temp/{resourceKey}";
                                ProcessCapturedObject(zipFilePath, destinationFolder);
                            }
        ...
        static void ProcessCapturedObject(string zipFilePath, string destination)
        {
            var fastZip = new FastZip();
            fastZip.ExtractZip(zipFilePath, destination, null);

#if OBJECT_CAPTURE_AVAILABLE && UNITY_EDITOR_OSX
            // Showing the object capture window and starting processing in the same frame was causing crashes
            EditorApplication.delayCall += () =>
            {
                var objectCaptureWindow = EditorWindow.GetWindow<ObjectCaptureWindow>();
                objectCaptureWindow.Show();
                EditorApplication.delayCall += () =>
                {
                    objectCaptureWindow.StartProcessing(destination);
                };
            };
#else
            Debug.Log($"Capture images extracted to {destination}");
#endif
        }

        static GameObject ImportEnvironment(string environmentName, string jsonText, string path,
            GameObject simulatedPlanePrefab, Material simulatedPlaneMaterial, Post postPrefab, MeshFilter meshPrefab,
            GameObject simulatedLightingPrefab)
        {
            ...
            {
                var planeVertices = plane.vertices ?? new List<Vector3>();
                ...
                synthesizedPlane.SetMRPlaneData(planeVertices, plane.center, plane.extents);
        ...
        static IEnumerator ImportDataRecording(this IUsesCloudStorage storageUser, string recordingPath,
            string recordingKey, string recordingName, string jsonText, float lagCompensation, ResourceSyncState state,
            Action<UnityObject> callback)
        {
            ...
            {
                resourceCount++;
                requests.Push(storageUser.DownloadCameraPath(recordingKey, (success, poseEvents) =>
                {
                    if (success && poseEvents != null)
                    {
                        var eventCount = poseEvents.Count;
                        if (eventCount > 0)
                        {
                            for (var i = 0; i < eventCount; i++)
                            {
                                var poseEvent = poseEvents[i];
                                poseEvent.time += lagCompensation;
                                poseEvents[i] = poseEvent;
                            }

                            k_CameraPoseRecorder.PoseEvents = poseEvents;
            ...
            requests.Push(storageUser.DownloadVideo(recordingKey, (_, path) =>
            {
                try
                {
                    AssetDatabase.StartAssetEditing();
