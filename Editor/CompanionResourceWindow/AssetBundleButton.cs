...
namespace Unity.AR.Companion.Core
{
    class AssetBundleButton : Button
    {
            ...
            UnityObject asset, IUsesCloudStorage storageUser)
        {
            ...
            if (state == State.NotUploaded && asset == null)
                SetEnabled(false);
        ...
        public void Process(ResourceList resourceList, List<AssetBundleBuild> builds,
            Dictionary<string, AssetImporter> importers)
        {
            switch (m_State)
            {
                case State.NotUploaded:
                    ...
                    m_StorageUser.CloudSaveAsync(m_Bundle.Key, string.Empty, true);
                    return;
                case State.Uploaded:
                    if (m_Bundle != null)
                        return;
        ...
        internal bool PublishBundleAndCleanUp(string resourceFolder, UploadComplete complete, UploadProgress progress)
        {
            ...
            switch (m_Resource.type)
            {
                case ResourceType.Scene:
                    m_StorageUser.UploadSceneAssetBundle(resourceFolder, m_Platform.ToString(), m_Guid, OnComplete, OnUploadProgress);
                    break;
                case ResourceType.Prefab:
                    m_StorageUser.UploadPrefabAssetBundle(resourceFolder, m_Platform.ToString(), m_Guid, OnComplete, OnUploadProgress);
                    break;
                default:
