...
namespace Unity.AR.Companion.CloudStorage
{
    class GenesisCloudStorageModuleTests
    {
        ...
        const string k_Data2 = "{\"$type\" \"Unity.AR.Companion.Core.ResourceList, Unity.AR.Companion.Core\",\"m_Resources\" {\"$type\" \"System.Collections.Generic.List`1[System.Object], mscorlib\", \"$elements\" [\"class\", \"null\", \"!@#$%^&*()\\\\_+-=,.<>/?|:;\", \"private\", \"void\"]},\"m_BundleResources\" {\"$type\" \"System.Collections.Generic.List`1[System.Object], mscorlib\",\"$elements\" []}}"; // Test including nonsense special characters and words
        static readonly byte[] k_Data3 = { 54, 68, 69, 73, 20, 69, 73, 20, 61, 20, 74, 65, 73, 74 }; // UTF8 encoded "This is a test"

        OAuthTokenGenesisModule m_UnityIdentity;
        ...
        {
            m_UnityIdentity = new OAuthTokenGenesisModule();
            var loginTask = m_UnityIdentity.SignIn(intent: k_TestIntent);
        ...
        static void TestIntentFormattingForMars(string intent)
        {
            if (!intent.StartsWith("mars://on-registration-completed/"))
                Assert.Ignore("Intent is not formatted for AR Companion launch. Is the intent present?");
        }
        ...
        public IEnumerator SaveDataInCloud()
        {
            ...
            m_Module.IdentityProvider = m_UnityIdentity;
            m_Module.CloudSaveAsync(k_Key1, k_Data1, true,
                (saveSuccess, responseCode, response) =>
                {
        ...
        public IEnumerator SaveDataInCloudAndLoadItBack()
        {
            ...
            m_Module.CloudSaveAsync(k_Key2, k_Data2, true,
                (saveSuccess, responseCode, response) =>
                {
                    Assert.True(saveSuccess);
                    Assert.AreEqual(200L, responseCode);
                    m_Module.CloudLoadAsync(k_Key2, true,
                        (bool loadSuccess, long loadResponseCode, string dataLoaded) =>
                        {
                            testComplete = true;
        ...
        public IEnumerator SaveByteDataInCloudAndLoadItBack()
        {
            ...
            var testComplete = false;
            m_Module.IdentityProvider = m_UnityIdentity;
            m_Module.CloudSaveAsync(k_Key3, k_Data3, true,
                (saveSuccess, responseCode, response) =>
                {
                    Assert.True(saveSuccess);
                    Assert.AreEqual(200L, responseCode);
                    m_Module.CloudLoadAsync(k_Key3, true,
                        (bool loadSuccess, long loadResponseCode, byte[] dataLoaded) =>
                        {
                            testComplete = true;
                            Assert.True(loadSuccess);
