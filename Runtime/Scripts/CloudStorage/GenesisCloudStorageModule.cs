...
{
    [ScriptableSettingsPath("Assets/XR/UserSettings")]
        ...
        IModuleBehaviorCallbacks, IProvidesCloudStorage, IUsesIssueHandling
    {
        ...
        const int k_RequestHandshakeTimeout = 30;
...
#pragma warning restore 649, 414
        ...
        internal IUnityIdentity IdentityProvider { get; set; }
        ...
        /// <param name="showIssueDialog">Whether to show issue dialogs if the request fails</param>
        ...
        public RequestHandle CloudSaveAsync(string key, string serializedObject, bool showIssueDialog,
            Action<bool, long, string> callback = null, ProgressCallback progress = null, int timeout = CloudStorageDefaults.DefaultTimeout)
        {
            return CloudSaveAsyncBytes(key, Encoding.UTF8.GetBytes(serializedObject), showIssueDialog, timeout,
                callback, progress, ContentTypes.String);
        }
        ...
        /// <param name="timeout">The timeout duration (in seconds) for this request</param>
        /// <returns>A handle to this request which can be used to cancel it</returns>
        public RequestHandle CloudSaveAsync(string key, byte[] bytes, bool showIssueDialog, Action<bool, long, string> callback = null,
            ProgressCallback progress = null, int timeout = CloudStorageDefaults.DefaultTimeout)
        {
            return CloudSaveAsyncBytes(key, bytes, showIssueDialog, timeout, callback, progress);
        }
        ...
        /// </summary>
        /// <param name="key"> string that uniquely identifies this instance of the type. </param>
        /// <param name="path">Path to the file being saved</param>
        /// <param name="showIssueDialog">Whether to show issue dialogs if the request fails</param>
        /// <param name="callback"> a callback when the asynchronous call is done to show whether it was successful,
        /// with the response code and string. </param>
        /// <param name="progress">Called every frame while the request is in progress with two 0-1 values indicating
        ...
        /// <param name="timeout">The timeout duration (in seconds) for this request</param>
        /// <returns>A handle to this request which can be used to cancel it</returns>
        public RequestHandle CloudSaveFileAsync(string key, string path, bool showIssueDialog, Action<bool, long, string> callback = null,
            ProgressCallback progress = null, int timeout = CloudStorageDefaults.DefaultTimeout)
        {
            return CloudSaveAsyncFile(key, path, showIssueDialog, timeout, callback, progress);
        }

        RequestHandle CloudSaveAsyncBytes(string key, byte[] bytes, bool showIssueDialog, int timeout, Action<bool, long, string> callback,
            ProgressCallback progress, ContentTypes contentType = ContentTypes.ByteArray)
        {
            ...
            var requestToGenesis = GenesisRequestBuilder(key, httpMethod, showIssueDialog);
        ...
        RequestHandle CloudSaveAsyncFile(string key, string path, bool showIssueDialog, int timeout,
            Action<bool, long, string> callback, ProgressCallback progress)
        {
            if (!IsValidKey(key))
            {
                Debug.LogWarning($"Invalid cloud storage key: {key}");
            ...
            SendGenesisWebRequest(requestHandle, requestToGenesis, (genesisSuccess, genesisResponseCode, genesisBytes) =>
            {
                ...
                if (genesisBytes != null)
                {
                    ...
                    var requestToGoogle = FileUploadRequestBuilder(securedObject.url, timeout, path);
        ...
        UnityWebRequest GenesisRequestBuilder(string key, string httpMethod, bool showIssueDialog)
        {
            var projectId = m_ProjectIdentifier;
            if (!ProjectIdentifierIsValid(projectId))
                return null;

            if (IdentityProvider == null || !IdentityProvider.IsValid)
            {
                if (showIssueDialog)
        ...
        /// upload and download progress, respectively</param>
        /// <param name="timeout">The timeout duration (in seconds) for this request</param>
        public RequestHandle CloudLoadAsync(string key, bool showIssueDialog, Action<bool, long, string> callback, ProgressCallback progress = null,
            int timeout = CloudStorageDefaults.DefaultTimeout)
        {
            ...
            return CloudLoadAsyncBytes(key, showIssueDialog, ConvertResponseToString, timeout, progress, ContentTypes.String);
        }
        ...
        /// upload and download progress, respectively</param>
        /// <param name="timeout">The timeout duration (in seconds) for this request</param>
        public RequestHandle CloudLoadAsync(string key, bool showIssueDialog, Action<bool, long, byte[]> callback,
            ProgressCallback progress = null, int timeout = CloudStorageDefaults.DefaultTimeout)
        {
            return CloudLoadAsyncBytes(key, showIssueDialog, callback, timeout, progress);
        }

        RequestHandle CloudLoadAsyncBytes(string key, bool showIssueDialog, Action<bool, long, byte[]> callback,
            int timeout, ProgressCallback progress, ContentTypes contentType = ContentTypes.ByteArray)
        {
            if (callback == null)
            {
            ...
            var requestToGenesis = GenesisRequestBuilder(key, UnityWebRequest.kHttpVerbGET, showIssueDialog);
            if (requestToGenesis == null)
            {
                progress?.Invoke(1, 1);
                callback(false, 0, null);
        ...
        /// upload and download progress, respectively</param>
        /// <param name="timeout">The timeout duration (in seconds) for this request</param>
        public RequestHandle CloudLoadTextureAsync(string key, bool showIssueDialog, LoadTextureCallback callback, ProgressCallback progress = null,
            int timeout = CloudStorageDefaults.DefaultTimeout)
        {
            return CloudLoadAsyncTexture(key, showIssueDialog, callback, timeout, progress);
        }
        ...
        RequestHandle CloudLoadAsyncTexture(string key, bool showIssueDialog, LoadTextureCallback callback, int timeout, ProgressCallback progress)
        {
            if (callback == null)
            {
        ...
        static UnityWebRequest FileUploadRequestBuilder(string url, int timeout, string path)
        {
            var request = new UnityWebRequest(new Uri(url), UnityWebRequest.kHttpVerbPUT);
            request.timeout = timeout;
            request.SetRequestHeader("Content-Type", HttpContentTypes.ApplicationOctetStream);
            request.SetRequestHeader("Cache-Control", k_CacheControlHeaderOptions);
    ...
    static class CloudStorageExtensions
    {
        /// <summary>
        /// Check the current authentication status
        /// </summary>
        /// <param name="user">The functionality user</param>
        /// <returns>True if the user is signed in with a valid identity</returns>
        internal static bool HasValidIdentity(this IUsesCloudStorage user)
        {
            if (!(user.provider is GenesisCloudStorageModule genesisModule))
                return false;

            var identityProvider = genesisModule.IdentityProvider;
            return identityProvider != null && identityProvider.IsValid;
        }
    }
}
