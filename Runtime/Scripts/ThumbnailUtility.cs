...
namespace Unity.AR.Companion.Core
{
    ...
    {
        static readonly Quaternion k_PreviewRotation = Quaternion.Euler(20, 140, 0);
        ...
        const float k_LightIntensity = 1.0f;
        const float k_Padding = 1.1f;
        static readonly Color k_BackgroundColor = Color.clear;
        static readonly Color k_AmbientLightColor = new Color(.65f, .65f, .65f, 0);
        const int k_IconSize = 512;
        ...
        public static byte[] GenerateMarkerThumbnail(Texture2D markerImage)
        {
